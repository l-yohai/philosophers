...
/*   Updated: 2020/08/15 04:59:20 by yohlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*monitor_death(void *ph)
{
	...
	while (1)
	{
		...
		if ((timestamp = get_timestamp_ms()) - philo->time_of_last_eat >
...
int		print_status(t_philo *philo, int index)
{
	...
	pthread_mutex_lock(&philo->mutex->write);
	pthread_mutex_unlock(&philo->mutex->global_died);
	print_msg(philo, index, get_timestamp_ms());
	pthread_mutex_unlock(&philo->mutex->write);
	return (1);
}
...
int		eat(t_philo *philo)
{
	...
	pthread_mutex_lock(&philo->last_eat);
	philo->time_of_last_eat = get_timestamp_ms();
	pthread_mutex_unlock(&philo->last_eat);
	usleep(philo->data->time_to_eat);
	pthread_mutex_unlock(&philo->mutex->fork[philo->id]);
	pthread_mutex_unlock(&philo->mutex->fork[philo->next_id]);
	if (++philo->meal_counter == philo->data->number_of_times_each_philosopher_must_eat)
	{
...
void	run_simulation(t_philo *philo, t_data *data, t_mutex *mutex)
{
	int			i;
	pthread_t	tid;
	...
	data->time_of_start = get_timestamp_ms();
	i = -1;
	while (++i < data->number_of_philosophers)
	{
...
int		main(int ac, char **av)
{
	...
	philo = NULL;
	if (!parse(av, &data) || !init(&philo, &mutex, data.number_of_philosophers))
		return (1);
	run_simulation(philo, &data, &mutex);
	cleanup(philo);
	return (0);
}